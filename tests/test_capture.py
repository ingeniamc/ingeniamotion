...
@pytest.mark.skip(reason="Bug in context manager in released version of ingenialink 7.5.1")
@pytest.mark.ethernet
@pytest.mark.soem
@pytest.mark.canopen
@pytest.mark.usefixtures("skip_if_monitoring_not_available")
...
@pytest.mark.virtual
def test_create_poller_exceptions(mc, alias, name, axis):
...
@pytest.mark.virtual
def test_create_empty_monitoring_exception(mocker, mc, alias):
...
@pytest.mark.virtual
def test_check_monitoring_version_v3(mc, alias):
...
@pytest.mark.virtual
def test_enable_monitoring_exception(mocker, mc, alias):
...
@pytest.mark.virtual
def test_get_monitoring_process_stage_v3(mocker, mc, alias, monitor_status, expected_stage):
...
@pytest.mark.virtual
def test_get_monitoring_process_stage_v1_v2(mocker, mc, alias, monitoring_status, expected_stage):
...
@pytest.mark.virtual
def test_is_frame_available(mocker, mc, alias, monitoring_status, monitoring_version):
    mocker.patch.object(mc.capture, "get_monitoring_status", return_value=monitoring_status)
...
@pytest.mark.virtual
def test_monitoring_disturbance_max_sample_size_exception(mocker, mc, alias, function):
    mocker.patch.object(mc.communication, "get_register", return_value="invalid_value")
    with pytest.raises(TypeError):
        getattr(mc.capture, function)(servo=alias)
...
@pytest.mark.virtual
def test_get_frequency_exception(mocker, mc, alias):
    mocker.patch.object(mc.communication, "get_register", return_value="invalid_value")
    with pytest.raises(TypeError):
