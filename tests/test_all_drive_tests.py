...
def assert_test_returns_to_baseline(servo: Servo, baseline: DriveRegistersValue):
...
def test_digital_halls_test(mc, alias, feedback_list, servo: Servo, baseline: DriveRegistersValue):
...
def test_incremental_encoder_1_test(
    mc, alias, feedback_list, servo: Servo, baseline: DriveRegistersValue
):
    commutation_fdbk = mc.configuration.get_commutation_feedback(servo=alias)
...
@pytest.mark.not_valid_for_product(part_number="CAP-XCR-E")
def test_secondary_ssi_test(mc, alias, feedback_list, servo: Servo, baseline: DriveRegistersValue):
    commutation_fdbk = mc.configuration.get_commutation_feedback(servo=alias)
    if SensorType.QEI in feedback_list:
...
def test_commutation(
    alias: str, mc: "MotionController", servo: Servo, baseline: DriveRegistersValue
) -> None:
...
@pytest.mark.soem
@pytest.mark.canopen
def test_commutation_error(mc, alias, force_fault, servo: Servo, baseline: DriveRegistersValue):
...
def test_phasing_check(mc, alias, servo: Servo, baseline: DriveRegistersValue):
    mc.tests.commutation(servo=alias)
...
@pytest.mark.soem
@pytest.mark.canopen
def test_phasing_check_error(mc, alias, force_fault, servo: Servo, baseline: DriveRegistersValue):
    with pytest.raises(force_fault):
...
@pytest.mark.soem
@pytest.mark.canopen
def test_sto_test(mc, alias, servo: Servo, baseline: DriveRegistersValue):
...
def test_sto_test_error(
    mocker, mc, alias, sto_value, message, servo: Servo, baseline: DriveRegistersValue
):
...
def test_sto_test_logs(
    caplog, mocker, mc, alias, sto_value, servo: Servo, baseline: DriveRegistersValue
):
...
@pytest.mark.soem
@pytest.mark.canopen
def test_brake_test(mc, alias, servo: Servo, baseline: DriveRegistersValue):
...
@pytest.mark.not_valid_for_product(part_number="CAP-XCR-E")
def test_feedback_stop(mc, alias, feedback_class, servo: Servo, baseline: DriveRegistersValue):
...
@pytest.mark.virtual
def test_commutation_stop(mc, alias, servo: Servo, baseline: DriveRegistersValue):
...
@pytest.mark.virtual
def test_phasing_check_stop(mc, alias, servo: Servo, baseline: DriveRegistersValue):
...
def test_current_ramp_up(
    mc, alias, test_currents, test_sensor, servo: Servo, baseline: DriveRegistersValue
):
    axis = 1
