...
cdef CyIpAddreString _parse_ip_addr_string(IP_ADDR_STRING data):
...
cdef CyAdapterType _parse_adapter_type(IP_ADAPTER_INFO* adapter):
...
cdef list _parse_adapters(IP_ADAPTER_INFO* adapter_info):
    cdef IP_ADAPTER_INFO* current_adapter = adapter_info
    adapters_list = []

    while current_adapter:
        if current_adapter.DhcpEnabled:
            dchp_server = _parse_ip_addr_string(current_adapter.DhcpServer)
        else:
            dchp_server = None
        parsed_adapter = CyAdapter(
            ...
            Type=_parse_adapter_type(current_adapter),
            DhcpEnabled=current_adapter.DhcpEnabled,
            IpAddressList=_parse_ip_addr_string(current_adapter.IpAddressList),
            GatewayList=_parse_ip_addr_string(current_adapter.GatewayList),
            DhcpServer=dchp_server,
            HaveWins=current_adapter.HaveWins,
            PrimaryWinsServer=_parse_ip_addr_string(current_adapter.PrimaryWinsServer),
            SecondaryWinsServer=_parse_ip_addr_string(current_adapter.SecondaryWinsServer),
...
def get_adapters_info():
    ...
    adapters = _parse_adapters(adapter_info)
