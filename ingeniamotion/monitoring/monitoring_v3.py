...
class MonitoringV3(Monitoring):
    ...
    def set_trigger(self, trigger_mode, edge_condition=None,
        ...
        if trigger_mode == MonitoringSoCType.TRIGGER_EVENT_EDGE:
            ...
            self.__rising_or_falling_edge_trigger(edge_condition,
                                                  index_reg, level_edge)

    def __rising_or_falling_edge_trigger(self, edge_condition,
                                         index_reg, level_edge):
        self.mc.communication.set_register(
            self.MONITOR_START_CONDITION_CONFIG_REGISTER,
            edge_condition, servo=self.servo, axis=0)
        self.mc.communication.set_register(
            self.MONITORING_TRIGGER_THRESHOLD_REGISTER,
            level_edge, servo=self.servo, axis=0)
        self.mc.communication.set_register(
            self.MONITORING_INDEX_CHECKER_REGISTER,
            index_reg, servo=self.servo, axis=0)
    ...
    def configure_number_samples(self, total_num_samples, trigger_delay_samples):
        ...
        self._check_buffer_size_is_enough(total_num_samples, trigger_delay_samples,
    ...
    def _check_monitoring_is_ready(self):
        ...
        if not is_ready:
            ...
            result_text = ("Can't read monitoring data because monitoring is not ready."
                           " Monitoring stage is {}. Monitoring is {}."
                           .format(monitoring_stage.name, text_is_enabled))
    ...
    def read_monitoring_data(self, timeout=None, progress_callback=None):
        ...
        data_array = super().read_monitoring_data(timeout=timeout,
    ...
    def _check_data_is_ready(self):
        ...
        data_is_ready &= self.mc.capture.is_frame_available(
            self.servo, version=self._version)
        return data_is_ready

    def rearm_monitoring(self):
        """Rearm monitoring."""
        self.mc.communication.set_register(
            self.MONITORING_REARM_REGISTER,
            1,
            servo=self.servo,
            axis=0
        )
